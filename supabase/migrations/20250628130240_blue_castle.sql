...
CREATE POLICY "HR users can manage candidates" ON candidates
  FOR ALL TO authenticated 
  USING (
    EXISTS (
      SELECT 1 FROM users 
...
CREATE POLICY "HR users can manage comments" ON comments
  FOR ALL TO authenticated 
  USING (
    EXISTS (
      SELECT 1 FROM users 
...
CREATE POLICY "HR users can manage reminders" ON reminders
  FOR ALL TO authenticated 
  USING (
    EXISTS (
      SELECT 1 FROM users 
...
('admin@empresa.com', 'Administrador RH', 'Administrador', 'Recursos Humanos')
